...
apps/web/
  ...
  src/
    ...
    routes/
      ...
      _dashboard/
        ...
        project/
          ...
          $projectId/
            layout.tsx                                                   ← CREATE (ProjectShell)
    ...
    features/
      ...
      environment-switcher/                                              ← DELETE (entire folder)
      env-switcher/
...
## Task 3: Delete playground route and the old environment-switcher feature

**Files:**
- Delete: `apps/web/src/routes/_dashboard/playground.tsx`
- Delete: `apps/web/src/features/environment-switcher/` (entire folder)

- [ ] **Step 1: Confirm no imports reference these paths**

```bash
cd apps/web && grep -rn "features/environment-switcher" src/ || echo "clean"
grep -rn "_dashboard/playground" src/ || echo "clean"
```
Both should print "clean". If anything matches, fix or delete the importer first.
...
```
This runs `tsr generate && tsc --noEmit`. Expected: clean type-check; the regenerated `routeTree.gen.ts` no longer references `playground`.

- [ ] **Step 4: Commit**

```bash
git add apps/web/src/routes/_dashboard apps/web/src/features apps/web/src/routeTree.gen.ts
git commit -m "chore(web): remove playground route and old environment-switcher"
```
...
## Task 13: Restructure project routes under `$projectId/` and add ProjectShell

**Files:**
- Keep: `apps/web/src/routes/_dashboard/project/layout.tsx` (still bare `<Outlet/>`)
- Create: `apps/web/src/routes/_dashboard/project/$projectId/layout.tsx`
- Create: `apps/web/src/routes/_dashboard/project/$projectId/index.tsx`

The existing `/_dashboard/index.tsx` already navigates to `/project/$projectId` after creating a project, so the route shape stays compatible. We add the `$projectId/` folder and put the project shell at its layout.

- [ ] **Step 1: Create `apps/web/src/routes/_dashboard/project/$projectId/layout.tsx`**
...
- [ ] **Step 2: Create the canvas placeholder `$projectId/index.tsx`**
...
- [ ] **Step 3: Type-check + verify route generation**

```bash
cd apps/web && bun run check-types
```
Expected: `routeTree.gen.ts` regenerates with `/_dashboard/project/$projectId` and `/_dashboard/project/$projectId/`.
...
```bash
git add apps/web/src/routes/_dashboard/project apps/web/src/routeTree.gen.ts
git commit -m "feat(web): project shell layout + canvas placeholder route"
```
