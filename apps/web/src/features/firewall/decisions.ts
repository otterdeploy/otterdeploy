...
/** Is this decision an IP ban on `ip`? The unit `unblock` works in: it clears
 *  every decision targeting one address, so the local delete has to find them
 *  all the same way. */
function bansIp(d: Decision, ip: string): boolean {
...
// Module-private on purpose: nothing outside this file holds the collection.
// Everything a view legitimately wants from it — the rows, the banned set, the
// three writes — is exported below with the permission gate and the toasts
// already attached, which is the only shape that is safe to hand out.
const decisionsCollection = createCollection(decisionsCollectionOptions);
