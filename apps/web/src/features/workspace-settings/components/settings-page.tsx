    ...
    <div className="grid gap-8 p-6">
      <div className="grid gap-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          Global credentials, ingress behavior, backup destinations, certificates, and alerting live here so each project can stay focused on app-level config.
        </p>
      </div>
      ...
      <div className="grid gap-10">
        <Section id="overview" title="Overview">
          <Field>
            ...
            <FieldDescription>Workspace metadata persistence lands with the settings API.</FieldDescription>
          </Field>
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard label="Admin URL" value="ops.otterstack.local" sub="Pinned to the control plane ingress" />
            <StatCard label="Release channel" value="beta" sub="Auto-upgrades paused pending maintenance window" />
            <StatCard label="Audit retention" value="30 days" sub="Applies to deploy, restore, and admin actions" />
          </div>
        </Section>
        <Section id="account" title="Account">
          <div className="grid gap-4 md:grid-cols-2">
            <StatCard label="CLI tokens" value="2 active" sub="One local dev token and one CI token are provisioned" />
            <StatCard label="Sessions" value="3 devices" sub="MacBook Pro, staging bastion, and browser session" />
          </div>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell><Badge variant="outline">rotates weekly</Badge></TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </Section>
        <Section id="profile" title="Profile">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field>
              ...
              <Input id="profile-email" defaultValue="jefferson@otterstack.dev" disabled />
            </Field>
            <Field>
            ...
            <Field>
              ...
              <Input id="profile-2fa" defaultValue="TOTP enabled · recovery codes stored" disabled />
            </Field>
          </div>
        </Section>
        <Section id="web-server" title="Web Server">
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard label="Ingress engine" value="Caddy" sub="Global HTTP/TLS entrypoint for all public routes" />
            <StatCard label="ACME issuer" value="Let's Encrypt" sub="Wildcard certs staged through DNS challenge" />
            <StatCard label="Reload cadence" value="hot reload" sub="Config changes apply without dropping sockets" />
          </div>
          <Field>
...
*.otterstack.local {
  ...
  tls {
    dns cloudflare {env.CLOUDFLARE_API_TOKEN}
            ...
            />
            <FieldDescription>Route-level edits still happen from workspace routing and per-project networking.</FieldDescription>
          </Field>
        </Section>
            ...
            <TableBody>
              {[
                ...
                <TableRow key={name}>
                  ...
                  <TableCell className="font-mono text-xs text-muted-foreground">{fingerprint}</TableCell>
        ...
        <Section id="certificates" title="Certificates">
          <Table>
            ...
            <TableBody>
              {[
                ["*.otterstack.local", "Let's Encrypt", "in 67 days", "healthy"],
        ...
        <Section id="cluster" title="Cluster">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard label="Image pull cache" value="enabled" sub="Warm on managers and edge nodes" />
            <StatCard label="Log retention" value="14 days" sub="Container + access logs rotated nightly" />
          </div>
          <Field>
...
- pin GPU workloads to node label gpu=true`}
        ...
        <Section id="notifications" title="Notifications">
          <ConnectionTable
            rows={[
              ...
              ["Email", "oncall@otterstack.dev", "digest + critical alerts", "healthy"],
        ...
        <Section id="identity" title="Identity & SSO">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <StatCard label="SCIM" value="not enabled" sub="Provisioning waits on org lifecycle hooks" />
          </div>
        </Section>
        <Section id="billing" title="Billing">
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard label="Plan" value="Enterprise self-hosted" sub="Includes audit retention and SSO" />
            <StatCard label="Current usage" value="12 projects" sub="5 remote servers · 43 deployed services" />
            <StatCard label="Next invoice" value="May 31" sub="Usage export can be pushed to finance" />
          </div>
        </Section>
        <Section id="danger" title="Danger zone">
          <div className="flex flex-wrap gap-3">
            <Button variant="destructive" disabled>Delete workspace</Button>
            <Button variant="outline" disabled>Rotate all credentials</Button>
            <Button variant="outline" disabled>Pause ingress</Button>
          </div>
