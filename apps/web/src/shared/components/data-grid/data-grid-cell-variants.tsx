...
import { DataGridCellWrapper } from "@/shared/components/data-grid/data-grid-cell-wrapper";
...
import { useBadgeOverflow } from "@/shared/components/data-grid/hooks/use-badge-overflow";
...
export function ShortTextCell<TData>({
  ...
  return (
    ...
    >
      ...
      {(() => {
        const fk = tableMeta?.fks?.[columnId];
        if (isEditing || !fk || value == null || value === "") return null;
        return (
          <button
            type="button"
            ...
            onClick={(e) => {
              e.stopPropagation();
              tableMeta?.onFkOpen?.(fk, String(value), e.currentTarget);
            }}
            className="absolute inset-y-0 right-1 my-auto flex size-5 items-center justify-center rounded text-muted-foreground/60 transition-colors hover:bg-muted hover:text-foreground"
          >
...
export function LongTextCell<TData>({
  ...
  // Base UI's Popup has no onOpenAutoFocus; focus + pending-char insert when the
  // editor opens (isEditing) instead. rAF lets the portalled popup mount first.
  React.useEffect(() => {
    if (!isEditing) return;
    const focus = () => {
  ...
  return (
    ...
    >
      <Checkbox
        ...
        onMouseDown={
          onCheckboxMouseDown as unknown as React.ComponentProps<
            typeof Checkbox
          >["onMouseDown"]
        }
        onDoubleClick={
          onCheckboxDoubleClick as unknown as React.ComponentProps<
            typeof Checkbox
          >["onDoubleClick"]
        }
      />
...
export function SelectCell<TData>({
  ...
  const onValueChange = React.useCallback(
    // Base UI Select passes (value, eventDetails); value can be null.
...
export function MultiSelectCell<TData>({
  ...
  // Base UI Popup has no onOpenAutoFocus; focus the input when the editor opens.
  React.useEffect(() => {
    if (!isEditing) return;
...
export function FileCell<TData>({
  ...
  // Base UI Popup has no onOpenAutoFocus/onEscapeKeyDown; focus the dropzone
  // when the editor opens. Escape is handled by onWrapperKeyDown + Base UI's
  // built-in dismiss.
  React.useEffect(() => {
    if (!isEditing) return;
