...
export const Route = createFileRoute("/_app/$orgSlug/settings/workspace/git-providers/$providerId")({
...
function GitProviderDetailRoute() {
  ...
  const del = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: orpc.git.list.queryKey({ input: undefined }) });
      toast.success("GitHub App deleted");
      void navigate({ to: "/$orgSlug/settings/workspace/git-providers", params: { orgSlug } });
    },
  ...
  return (
    <Page>
      <PageHeader
        ...
        actions={<DeleteButton pending={del.isPending} onDelete={() => del.mutate({ providerId: providerId as GitProviderId })} />}
      />
...
type ProviderData = Awaited<ReturnType<typeof orpc.git.getProvider.call>>;
...
function GeneralTab({ provider }: { provider: ProviderData }) {
    ...
    <div className="flex flex-col gap-6 rounded-lg border bg-card p-5">
      <dl className="grid gap-x-8 gap-y-4 sm:grid-cols-2">
        ...
        <Row
          label="Account"
          value={
            inst ? `${inst.accountType === "organization" ? "org" : "user"}/${inst.accountLogin}` : "—"
          }
          mono
        />
        ...
        <Row
          label="Repositories"
          value={
            inst ? `${inst.repoCount}${inst.repoSelection === "selected" ? " (selected)" : " (all)"}` : "—"
          }
        />
      ...
      <div className="flex flex-col gap-2 border-t pt-4">
        ...
        <p className="text-[11.5px] text-muted-foreground">
          Secrets are encrypted at rest and never shown. GitHub issued them when the App was created.
        </p>
      </div>
    </div>
  );
...
function PermissionsTab({ provider }: { provider: ProviderData }) {
  const inst = provider.installation;
    ...
    <div className="flex flex-col gap-4 rounded-lg border bg-card p-5">
      ...
      {entries.length === 0 ? (
        <p className="text-[12.5px] text-muted-foreground">
          {inst ? "No permissions recorded yet — try Refetch." : "Install the App to see its permissions."}
        </p>
      ) : (
...
function ResourcesTab({ orgSlug, providerId }: { orgSlug: string; providerId: GitProviderId }) {
