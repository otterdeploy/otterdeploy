...
// Ported from /tmp/anth-design-qP3sS7/otterdeploy/project/screens3.jsx (Networking3).
        ...
        <div className="card" style={{ padding: 18, marginTop: 12 }}>
          <Field label="Sink">
            <select className="input">
              <option>otterdeploy-logs.helio.internal:5170</option>
...
function buildCaddyfileFull(routes: Route[]): string {
  const L: string[] = [];
  L.push("# Auto-generated by Otterdeploy — edit here OR via the Routes table");
  ...
  for (const r of routes.filter((x) => x.publicHosts.length > 0)) {
    ...
    L.push("\t}");
    L.push("\tlog {");
    L.push("\t\toutput net otterdeploy-logs.helio.internal:5170");
    L.push("\t\tformat json");
    L.push("\t}");
    L.push("}");
    L.push("");
