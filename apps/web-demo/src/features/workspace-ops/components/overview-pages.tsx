import { AlertTriangleIcon, ArrowUpRightIcon, Clock3Icon, ContainerIcon, GaugeIcon, HardDriveIcon, Layers3Icon, NetworkIcon, ServerIcon, WorkflowIcon } from "lucide-react";
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="CPU saturation" value="61%" detail="2 nodes above target, autoscale queued" icon={<GaugeIcon className="size-4" />} />
        <MetricCard title="Ingress p95" value="182 ms" detail="Requests trending down after last deploy" icon={<ArrowUpRightIcon className="size-4" />} />
        <MetricCard title="Certificates due" value="1" detail="One custom PEM chain expires in 11 days" icon={<Clock3Icon className="size-4" />} />
        <MetricCard title="Open incidents" value="2" detail="One degraded route, one slow image pull" icon={<AlertTriangleIcon className="size-4" />} />
      </div>
      <div className="grid gap-4 xl:grid-cols-[1.3fr_1fr]">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>High-signal issues surfaced from builds, ingress, and backup jobs.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 pt-0">
            <AlertRow title="preview-builder image pull slowness" detail="Registry auth fallback added 13s to the last deploy." status="watch" />
            <AlertRow title="console.paperhouse.dev certificate" detail="Custom chain expires soon and needs rotation." status="renew soon" />
            <AlertRow title="nightly volume backup" detail="Completed with retry against the cold-storage destination." status="healthy" />
          </CardContent>
        </Card>
      </div>
    </div>
    ...
    <div className="grid gap-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Caddy access log stream for public routes. Retention and export policies will plug into this view.</CardDescription>
        </CardHeader>
        <CardContent className="pt-0">
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {[
                ...
                ["GET", "preview.otterstack.dev", "/assets/index.js", "503", "861 ms", "GB"],
                ...
                <TableRow key={`${method}-${host}-${path}`}>
                  ...
                  <TableCell><StatusCodeBadge code={status} /></TableCell>
...
export function DockerResourcesOverview() {
  return (
    <div className="grid gap-4">
      ...
      />
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="Running containers" value="28" detail="3 unmanaged sidecars are flagged for review" icon={<ContainerIcon className="size-4" />} />
        <MetricCard title="Images cached" value="94" detail="17 are older than 14 days" icon={<Layers3Icon className="size-4" />} />
        <MetricCard title="Persistent volumes" value="32" detail="6 detached volumes can be pruned" icon={<HardDriveIcon className="size-4" />} />
        <MetricCard title="Networks" value="11" detail="2 bridge networks are non-project scoped" icon={<NetworkIcon className="size-4" />} />
      </div>
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Containers</CardTitle>
            <CardDescription>Useful when something is stuck outside the Stack abstraction.</CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {[
                  ...
                  <TableRow key={name}>
                    ...
                    <TableCell className="font-mono text-xs text-muted-foreground">{image}</TableCell>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Track attach state, ownership, and backup eligibility.</CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            <Table>
              <TableHeader>
...
export function SwarmOverview() {
  return (
    <div className="grid gap-4">
      ...
      />
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="Managers" value="3 / 3" detail="Raft quorum healthy" icon={<ServerIcon className="size-4" />} />
        <MetricCard title="Workers" value="5" detail="1 node draining for maintenance" icon={<WorkflowIcon className="size-4" />} />
        <MetricCard title="Services" value="43" detail="2 canary rollouts active" icon={<Layers3Icon className="size-4" />} />
        <MetricCard title="Pending tasks" value="1" detail="Waiting on private registry auth refresh" icon={<Clock3Icon className="size-4" />} />
      </div>
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Nodes</CardTitle>
            <CardDescription>Placement, availability, and health at the swarm layer.</CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            <Table>
              <TableHeader>
