...
async function requireService(args: { service?: string; url?: string; slug?: string }) {
  ...
  const slug = args.slug ?? loadManifestFile().project;
  const project = await client.project.getBySlug({ slug });
  if (!args.service) {
    consola.error("--service <name> is required.");
    process.exit(1);
  }
  // Resolve service name → resourceId via the project's resource list.
...
const setEnv = defineCommand({
  ...
  async run({ args, rawArgs }) {
    ...
    if (pairs.length === 0) {
      consola.error("Pass at least one KEY=VAL pair, e.g. `env set --service web DATABASE_URL=postgres://...`");
      process.exit(1);
    }
...
function parseDotenv(body: string): Array<{ key: string; value: string }> {
  ...
  for (const rawLine of body.split(/\r?\n/)) {
    ...
    // Strip wrapping quotes if symmetric.
    if (
