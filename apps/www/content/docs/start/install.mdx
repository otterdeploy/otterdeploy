...
description: Install the Otterdeploy control plane on a supported Linux server with one command that provisions the host and starts the stack.
---
...
Each published installer has a SHA-256 file at the same URL. Download both,
verify the release artifact, then run the local copy:

```bash
curl -fsSLO https://get.otterdeploy.com/install.sh
curl -fsSLO https://get.otterdeploy.com/install.sh.sha256
sha256sum --check install.sh.sha256
bash install.sh
```

The check fails closed if a new release lands between the two downloads: fetch
both files again and repeat it. Version-pinned artifacts are also available at
`https://get.otterdeploy.com/vX.Y.Z/install.sh` with the checksum beside them.

## What the host needs

| | |
| --- | --- |
| **OS** | Debian/Ubuntu, RHEL/Fedora-family, or Arch Linux (including recognised derivatives), with root or passwordless `sudo` |
| **Runtime** | Docker 28+; the installer adds Docker when it is absent |
