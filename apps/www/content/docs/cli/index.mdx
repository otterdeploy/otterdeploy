---
title: CLI
seoTitle: Install and use the CLI
description: Install and authenticate the otterdeploy CLI, then manage projects, services, databases, and deployments from your terminal or CI.
---

The `otterdeploy` CLI covers core project, service, database, domain,
deployment, log and backup workflows from your terminal and CI. It talks to
the same typed oRPC API as the web app.
...
Most commands operate on a project described by an `otterdeploy.json` (or
`otterdeploy.ts`) file in the working directory: a declarative manifest of
services, databases, and compose stacks. `deploy`/`sync`/`up` reconcile the live
project to match it; `pull` writes the server's manifest back to disk; `status`
shows drift.

Files named `otterdeploy.config.json` / `otterdeploy.config.ts` (the pre-0.8
spelling) are still resolved, so an existing repo keeps working, and commands
that rewrite the manifest (`add`, `pull`) keep writing to the file that is
already there. Only `init` picks the new name. To move over, rename the file.
...
- **`--json`**: machine-readable output where a command lists the flag, with no
  decoration. Streaming commands emit one JSON event per line.
