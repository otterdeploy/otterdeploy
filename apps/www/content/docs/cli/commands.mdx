...
description: "Every otterdeploy CLI command and flag, grouped the way the binary groups them: projects, services, databases, deployments and logs."
---
...
Rebuild a service or compose stack from the head of its bound branch. Unlike
`restart` (same image, new container), this produces a new build.
...
- `target` *(positional, required)*: service or compose stack name.
- `--config <path>`, `--slug <project>`, `--url <url>`.
...
- `--path <path>` *(required)*: absolute container path to mount, e.g. `/data`.
...
- `--service <name>` *(required)*.
- `--path <path>` *(required)*: absolute container path of the volume to detach.
- `--yes`: skip the confirmation prompt.
- `--config`, `--slug`, `--url`.
