...
import type { CraftDot } from "./craft";
import type { FieldNode, Spark } from "./field";

import { buildCraft } from "./craft";
import { buildField, CAM_DIST, FOCAL, makeGlowSprite, mulberry32, TILT, Z_FAR } from "./field";

/** How the scene is framed. `panel` is the tall desktop brand column; `band`
 *  the short mobile strip; `wide` the landing hero's shallow right-side slab,
 *  where the horizon must sit inside a much shorter canvas. */
export type HeroLayout = "panel" | "band" | "wide";
...
function framingFor(layout: HeroLayout) {
  ...
  // Shallow canvas: the horizon fraction has to absorb HORIZON_DROP · scale,
  // or the whole ground field projects below the bottom edge.
  if (layout === "wide") return { horizon: 0.4, craftX: 0.6, craftY: 0.34, craftR: 0.22 };
...
export function readSceneColors(s: Scene) {
  // The landing's od-lin scope is dark-only; tokens resolve against the canvas
  // element, and the accent falls back to the brand blue.
  s.fg = readToken(s.canvas, "--foreground", "#f7f8f8");
  s.accent = readToken(s.canvas, "--primary", "#3d7bfb");
  s.inkAlpha = 0.68;
  s.glowStrength = 0.34;
