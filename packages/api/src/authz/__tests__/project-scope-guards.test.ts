...
describe("enforceProjectScope", () => {
  test("selected key, in-scope project ⇒ allowed (no throw)", () => {
    expect(() =>
      enforceProjectScope(ctx(selectedKey(["proj_1"])), "proj_1"),
    ).not.toThrow();
  });
  ...
  test("falsy projectId ⇒ no-op (can't determine project)", () => {
    expect(() =>
      enforceProjectScope(ctx(selectedKey(["proj_1"])), null),
    ).not.toThrow();
    expect(() =>
      enforceProjectScope(ctx(selectedKey(["proj_1"])), undefined),
    ).not.toThrow();
  });
});
