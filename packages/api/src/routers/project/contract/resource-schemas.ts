 ...
 *
 * Resource = a database (postgres today), a service, or a compose stack. The
 * discriminated union here is the single shape returned by
 * `project.resource.list` and `project.resource.get`. Postgres-specific
 * schemas live in `./postgres`.
 */
...
export const serviceResourceSchema = z.object({
  ...
  // context menu can offer "Copy internal hostname" the same way the
  // database view already does.
  internalHostname: z.string(),
  publicEnabled: z.boolean(),
