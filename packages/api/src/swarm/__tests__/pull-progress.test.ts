...
describe("createPullLineSummarizer", () => {
  test("passes headline events through verbatim", () => {
    ...
    expect(
      s.push(event({ status: "Status: Downloaded newer image for postgres:18-alpine" })),
    ).toBe("Status: Downloaded newer image for postgres:18-alpine");
  });
  ...
  test("suppresses per-layer chatter between intervals, then emits one aggregate line", () => {
    ...
    try {
      ...
      expect(
        s.push(event({ id: "a", status: "Downloading", current: 1024 * 1024, total: 8 * 1024 * 1024 })),
      ).toBeNull();
      expect(
        s.push(event({ id: "b", status: "Downloading", current: 2 * 1024 * 1024, total: 4 * 1024 * 1024 })),
      ).toBeNull();

      // Past the interval: one line with the cross-layer byte sums.
      vi.advanceTimersByTime(2_001);
      expect(
        s.push(event({ id: "a", status: "Downloading", current: 3 * 1024 * 1024, total: 8 * 1024 * 1024 })),
      ).toBe("Pulling postgres:18-alpine: 5.0 MB of 12.0 MB");

      // Emitting resets the throttle — the very next layer event is quiet again.
      expect(
        s.push(event({ id: "b", status: "Downloading", current: 3 * 1024 * 1024, total: 4 * 1024 * 1024 })),
      ).toBeNull();
    } finally {
      vi.useRealTimers();
    }
