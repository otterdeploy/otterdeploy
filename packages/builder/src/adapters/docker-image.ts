...
export class DockerImageBuilder implements Builder {
  async build(input: BuildInput): Promise<Result<BuildOutput, Error>> {
    ...
    try {
      ...
      logs.push(`Pulling image: ${remoteImage}`);
      ...
      logs.push(`Image pulled successfully: ${pullResult.unwrap()}`);
      ...
      logs.push(`Tagged as: ${localVersioned}`);
      ...
      if (tagLatestResult.isErr()) {
        ...
        logs.push(`Tagged as: ${localLatest}`);
      }
